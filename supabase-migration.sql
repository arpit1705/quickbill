...
-- ============================================================
-- RPC: bulk_upsert_items_by_name — bulk create/update products
-- ============================================================
CREATE OR REPLACE FUNCTION bulk_upsert_items_by_name(
  p_rows JSONB
)
RETURNS TABLE (
  id UUID,
...
BEGIN
  ...
  LOOP
    ...
    v_unit := trim(v_row->>'unit');
    v_stock_qty := COALESCE((v_row->>'stock_qty')::NUMERIC, 0);
    v_low_stock_threshold := (v_row->>'low_stock_threshold')::NUMERIC;

    SELECT i.id INTO v_existing_id
    FROM items i
    WHERE lower(trim(i.name)) = lower(v_name)
    ORDER BY i.is_active DESC, i.created_at ASC
    LIMIT 1;

    IF v_existing_id IS NULL THEN
      INSERT INTO items (name, price, unit, stock_qty, low_stock_threshold, is_active)
      VALUES (v_name, v_price, v_unit, v_stock_qty, v_low_stock_threshold, true)
      RETURNING items.id, items.name INTO id, name;
      action := 'created';
      RETURN NEXT;
    ELSE
      UPDATE items
