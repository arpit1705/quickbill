...
import { Plus, Pencil, Trash2, Camera, X, Loader2, Download, FileUp } from "lucide-react";
...
import type { BulkUpsertItemsResult } from "@/lib/db";
...
export function InventoryView({
  ...
  onBulkImport: (rows: ParsedInventoryImport["validRows"]) => Promise<BulkUpsertItemsResult>;
}) {
  const { t } = useLang();
  ...
  const onPickImport = async (file: File) => {
    try {
      ...
      if (!parsed.validRows.length && parsed.rowErrors.length) {
        toast.error(t.importOnlyErrors);
      }
      setImportPreview(parsed);
    } catch (error) {
      const message = error instanceof Error ? error.message : t.importFailed;
      if (message.startsWith("Missing required columns:")) {
        toast.error(t.importMissingColumns(message.replace("Missing required columns:", "").trim()));
      } else if (message.includes(".csv and .xlsx")) {
  ...
  const runImport = async () => {
    ...
    try {
      setImporting(true);
      const result = await onBulkImport(importPreview.validRows);
      toast.success(t.importSuccess(result.created, result.updated));
    ...
    <div className="px-4 pt-4 pb-10">
      ...
      <div className="grid grid-cols-2 gap-2 mb-2">
        <button
          ...
          className="h-10 rounded-xl border border-border text-sm font-semibold hover:bg-muted transition flex items-center justify-center gap-2"
        >
        ...
        <button
          ...
          className="h-10 rounded-xl border border-border text-sm font-semibold hover:bg-muted transition flex items-center justify-center gap-2 disabled:opacity-60"
        >
          {parsingImport ? <Loader2 className="h-4 w-4 animate-spin" /> : <FileUp className="h-4 w-4" />}
          {parsingImport ? t.parsingImport : t.importInventory}
        </button>
      </div>
      <button
      ...
      {importPreview && (
        <ConfirmDialog
          ...
          message={t.importSummary(
            importPreview.totalRows,
            importPreview.validRows.length,
            importPreview.rowErrors.length
          )}
          confirmLabel={importing ? t.parsingImport : t.importNow}
          onCancel={() => setImportPreview(null)}
          onConfirm={() => void runImport()}
          confirmDisabled={!importPreview.validRows.length || importing}
          details={
            importPreview.rowErrors.length
              ? `${t.importErrorsTitle}: ${importPreview.rowErrors
                  .slice(0, 5)
                  .map((err) => `#${err.rowNumber} ${err.field} - ${err.message}`)
                  .join(" | ")}`
              : undefined
          }
        />
